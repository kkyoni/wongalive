...
<div class="row wrapper border-bottom white-bg page-heading">
	<div class="col-lg-12">
		<h2><i class="fa fa-youtube-play"></i> User Video Management</h2>
		<ol class="breadcrumb">
			<li class="breadcrumb-item">
				<a href="{{ route('admin.dashboard') }}">Home</a>
			</li>
			<li class="breadcrumb-item active">
				<strong>User Video Table</strong>
			</li>
		</ol>
	</div>
</div>
...
<script type="text/javascript">
	$(document).on("click","a.deletevideo",function(e){
		...
		swal({
			...
			if (isConfirm) {
				$.ajax({
					url:"{{route('admin.videos.delete',[''])}}"+"/"+id,
					type: 'post',
				...
				swal("Cancelled", "Your users video is safe :)", "error");
			}
		});
		return false;
	})
	...
	$(document).on("click",".changeStatusRecord",function(e){
		...
		swal({
			...
			if (isConfirm) {
				$.ajax({
					url:"{{ route('admin.videos.change_status') }}",
					type: 'post',
					data: {"_method": 'post',
					'id':id,
					"_token": "{{ csrf_token() }}"
